...
- **Business Logic**: Pure functions at the core, side effects isolated at the edges. Business logic lives in `src/modules/` — never in route handlers or components. Export plain functions from modules — no class-based services.
- **Database**: Database access only through repository files. SQLite for demo, transitioning to PostgreSQL.
